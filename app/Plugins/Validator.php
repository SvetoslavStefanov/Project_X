...
class Validator {
    ...
    public static function init() {
        ...
        self::$translations = $translations['validation'];
    }

    public static function getTranslationByName($name, $rule = '') {
        $conditionWithRules = ['min_length', 'max_length'];

        if (in_array($name, $conditionWithRules)) {
            return self::$translations[$name] . ' ' . $rule;
        }
    ...
    public static function validate($value, $field, $rules) {
        ...
        foreach ($rules as $condition => $rule) {
            if (!self::$condition($value, $rule)) {
                self::addError($field, self::getTranslationByName($condition, $rule));
            }
        }
